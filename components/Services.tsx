...
export default function Services() {
  return (
    <section id="services" className="py-20 md:py-32" style={{ borderTop: "1px solid var(--border)" }}>
      <Container>
        <div className="flex flex-col lg:flex-row gap-12 lg:gap-24">
          ...
          <div className="flex-1">
            <h2 className="text-3xl md:text-5xl font-bold leading-tight mb-4" style={{ color: "var(--cream)" }}>
              Fixed scope. Fast.<br className="hidden md:block" /> No surprises.
            </h2>
            <p className="text-base md:text-lg mb-12" style={{ color: "var(--dim)" }}>
              ...
              {services.map((s) => (
                <div key={s.title} className="p-8 md:p-10" style={{ background: "var(--bg)" }}>
                  <div className="flex items-start justify-between gap-4 mb-4">
                    <h3 className="text-lg font-bold leading-tight" style={{ color: "var(--cream)" }}>{s.title}</h3>
                    <span
                      className="font-mono text-xs shrink-0 px-3 py-1 rounded whitespace-nowrap"
                      style={{ color: "var(--yellow)", background: "rgba(218,255,0,0.08)" }}
                    >
                      {s.tag}
                    </span>
                  </div>
                  <p className="text-sm md:text-base leading-relaxed" style={{ color: "var(--dim)" }}>{s.desc}</p>
                </div>
              ))}
            </div>
          </div>
